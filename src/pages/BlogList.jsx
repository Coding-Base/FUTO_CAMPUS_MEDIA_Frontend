...
const toAbsolute = (u) => {
  if (!u) return null;
  if (/^https?:\/\//i.test(u)) return u; // Already Cloudinary or absolute URL
...
const buildFallback = (u) => {
  if (!u) return null;
  if (/^https?:\/\//i.test(u)) return u; // Absolute URL stays as-is
  const path = u.startsWith("/") ? u : `/${u}`;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            return (
              ...
              >
                ...
                <div className="p-6">
                  ...
                  {post.subtitle && (
                    <h3 className="font-semibold mt-2 text-gray-600">{post.subtitle}</h3>
                  )}
                  ...
                  <div className="mt-6 flex items-center justify-between">
                    ...
                    <Link
                      to={`/posts/${post.slug}`}
                      className="text-indigo-600 font-medium hover:text-indigo-800 transition-colors duration-300 flex items-center gap-1"
                    >
                      Read more
                      <svg
                        className="w-4 h-4"
        ...
        <aside className="space-y-8">
          <div className="bg-white p-6 rounded-xl shadow-md">
            <h4 className="font-bold text-xl mb-4 pb-2 border-b border-gray-200">
              Popular Now
            </h4>
            {posts.slice(0, 4).map((p) => {
              ...
              return (
                <Link
                  key={p.id}
                  to={`/posts/${p.slug}`}
                  className="flex gap-4 items-start py-3 group border-b border-gray-100 last:border-0"
                >
                  {thumb && (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-semibold text-gray-800 group-hover:text-indigo-600 transition-colors duration-300 truncate">
                      {p.title}
                    </div>
